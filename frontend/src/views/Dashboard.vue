...
onMounted(async () => {
  try {
      ...
      series: [
        {
          ...
          data: data.value.trend.map(t => t.tasks),
          itemStyle: { color: '#3498db' },
        },
        {
          name: '成本(¥)', type: 'bar', yAxisIndex: 1,
          data: data.value.trend.map(t => t.cost),
    ...
    ElMessage.error('总览数据加载失败：' + (e.message || '未知错误'))
  } finally {
    loading.value = false
  }
})
